...
def main():
    ...
    J_coil, n_iters, converged = _picard_phase(
        ...
        closure=lambda: None, max_iters=max_iters, min_iters=max_iters, scif_tol=0.5,
    ...
    print(f"FINAL: converged={converged} n_iters={n_iters} SCIF={scif_final:+.3f}mT",
          flush=True)
    for i, T_i in enumerate(ta["layer_T_fns"]):
        print(f"STATE layer{i}: sum={T_i.x.array.sum():.10e}  "
              f"absmax={np.abs(T_i.x.array).max():.10e}", flush=True)
    print(f"STATE A_h: sum={ta['A_h'].x.array.sum():.10e}  "
          f"absmax={np.abs(ta['A_h'].x.array).max():.10e}", flush=True)
    print(f"STATE rho_fn: sum={ta['rho_fn'].x.array.sum():.10e}", flush=True)
