...
monotonically INCREASING current from a cold start. Whether it behaves
the same way for DEcreasing current has never been tested. Per this
project's own established practice: do not trust `_picard_phase`'s own
EMA-based `converged` flag (it has repeatedly fired hundreds of
iterations before genuine settling elsewhere in this project) -- this
script forces full-length iteration at every step and saves the raw
per-iteration dB_rel/SCIF/T_max trace so that can be checked directly,
not inferred from a flag.

The down-schedule's last step targets ~2A, not exactly 0A -- T_amp =
I/(2*delta_SC) appears in a normalization denominator in the trace
diagnostics (T_max/amp), which would divide by zero at I=0 exactly.

Output: a single .npz under transient/full_validation_plots/data/,
same schema as full_ramp_run.py (so the existing plotting helpers apply
unchanged), just with 20 steps instead of 10.
"""
import os
import sys
...
def main():
    ...
    out_name = sys.argv[4] if len(sys.argv) > 4 else "full_ramp_up_down.npz"
    ...
    params.mesh_filename = f"{root}_fullupdown_{os.getpid()}{ext}"
    ...
    for step_idx, (dt, I_now) in enumerate(SCHEDULE):
        ...
        J_coil, n_iters, converged = _picard_phase(
            ...
            label=f"step{step_idx}_{branch}", verbose=False)
        ...
        step_summaries.append(dict(step=step_idx, branch=branch, dt=dt,
                                   I_now=I_now, n_iters=n_iters,
                                   finite=bool(finite), scif_mT=final_scif,
                                   dB_rel=final_dB_rel, T_max_amp=final_T_max,
                                   T_min_amp=final_T_min))
        print(f"  step={step_idx:2d} [{branch}] I={I_now:6.1f}A  n_iters={n_iters}  "
              f"finite={finite}  scif={final_scif:+8.3f}mT  dB_rel={final_dB_rel:.4f}  "
        ...
        # flush incrementally -- a multi-hour run must not lose everything
        # to a crash partway through (project convention, CLAUDE.md
        # "Operational lessons")
        ...
        partial = dict(
            ...
            n_layers=params.n_layers, n_turns=np.array(params.n_turns),
            step_summaries=step_summaries,
            snapshot_steps=sorted(k for k in SNAPSHOT_STEPS if k <= step_idx),
            completed_through_step=step_idx,
        )
        for i in range(params.n_layers):
            partial[f"T_max_amp_layer{i}"] = np.array(trace["T_max_amp"][i])
            partial[f"T_min_amp_layer{i}"] = np.array(trace["T_min_amp"][i])
        for key, snap in snapshots.items():
            for k, v in snap.items():
                partial[f"{key}__{k}"] = v
        np.savez(os.path.join(OUT_DATA_DIR, out_name), **partial)

    print(f"\nSaved full up+down ramp data to "
          f"{os.path.join(OUT_DATA_DIR, out_name)}", flush=True)
