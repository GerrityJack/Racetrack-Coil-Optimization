"""full_ramp_run.py -- 2026-08-06, full production-scale validation run:
extends multistep_ramp_check.py's 5-step/98A test to the FULL champion
design current (I_design=196A), 10 steps of +19.6A each, still at the
confirmed-working dt=60s, alpha=(0.03, 0.01), forced full-length
(min_iters=max_iters_per_step) throughout -- every lesson from today's
validation applied at once.

Unlike multistep_ramp_check.py, this saves:
  - the FULL per-iteration diagnostic trace within every step (not just
    the final value) -- SCIF, per-layer T_max/T_min, dB_rel -- for
    trend plots across the whole ramp.
  - complete field snapshots (T per layer, A, B, all with dof/cell
    coordinates) at a configurable subset of steps, for spatial plots.

Output: a single .npz under transient/full_validation_plots/data/,
containing everything the two plotting scripts need. No plotting is
done here.
"""
import os
import sys
...
# 10 steps of +19.6A, dt=60s throughout, reaching the full champion
# design current.
SCHEDULE = [(60.0, 19.6 * (i + 1)) for i in range(10)]
SNAPSHOT_STEPS = {0, 4, 9}  # first, middle, last -- keep data volume sane
...
def main():
    ...
    out_name = sys.argv[4] if len(sys.argv) > 4 else "full_ramp.npz"
    ...
    params.mesh_filename = f"{root}_fullramp_{os.getpid()}{ext}"
    ...
    # per-iteration trace across the WHOLE ramp (cumulative iteration index)
    ...
    for step_idx, (dt, I_now) in enumerate(SCHEDULE):
        ...
        print(f"  step={step_idx} I={I_now:.1f}A  n_iters={n_iters}  "
    ...
    print(f"\nSaved full ramp data to {out_path}", flush=True)
